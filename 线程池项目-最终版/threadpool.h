...
public:
	...
	~ThreadPool()
	{
		...
		notEmpty_.notify_all();//唤醒所有等待队列中阻塞的线程
		exitCond_.wait(lock, [&]()->bool {return threads_.size() == 0; });
		//调用时检查conditions，如果为false,则阻塞线程 并且调用lock.unlock()
	...
	//submittask返回值 返回值是用decltype推导出来的
	auto submitTask(Func&& func, Args&&... args) -> std::future<decltype(func(args...))>
	{
		// 打包任务，放入任务队列里面
		using RType = decltype(func(args...));//返回值类型
		//创建了一个package task对象  打包了一个返回值是Rtype 不带参数（用bind绑定了）的一个函数对象
		...
		{
			//两个中有一个不满足的就阻塞
			// 表示notFull_等待1s种，条件依然没有满足 返回值是false
	...
	void start(int initThreadSize = std::thread::hardware_concurrency())
	{
		...
		for (int i = 0; i < initThreadSize_; i++)
		{
			threads_[i]->start(); // 需要去执行一个线程函数  启动线程时，线程函数自动执行
...
private:
	...
	void threadFunc(int threadid)
	{
		...
		for (;;)
		{
			Task task;
			{
				// 先获取锁  创建一个名为lock的unique_lock对象  并将其锁定到名为taskQueMtx_的mutex对象上
				//这样就能保证同一时间只有一个线程能够访问互斥量保护的资源
				std::unique_lock<std::mutex> lock(taskQueMtx_);
